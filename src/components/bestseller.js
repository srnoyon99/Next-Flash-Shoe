...
const BestSeller = () => {
  ...
  const products = [
    { name: 'shakerdyhdrtfgydrtfudrftudrftudtu', price: '$100', color: 'Red', color1: 'Blue', color2: 'Green', size: '40', size1: '42', size2: '38' },
          ...
          <SplideSlide className={'cursor-pointer border-1 rounded-3xl border-gray-400/20 min-h-fit shadow-2xs '} key={index}>
            <div className="  rounded-3xl grid-rows-1 items-center justify-center">
              ...
              <div className="text-start pl-5 border-t-[1px] border-gray-400 py-2">
                ...
                <p className="text-gray-800 dark:text-white">Size: {item.size} | {item.size1} | {item.size2}</p>
              </div>
            </div>
          </SplideSlide>
        ))}
