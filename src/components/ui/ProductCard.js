...
export const ProductCard = ({
  ...
  imageContent,
  imageWrapperClassName = ' flex flex-col items-center justify-center',
  showSize = true,
}) => (
  <div className={innerClassName || (grid
    ...
    {showSize && <p className="text-gray-800 dark:text-white text-center "> {item.size} | {item.size1} | {item.size2}</p>}
  ...
  <Splide options={options}>
    {items.map((item, index) => (
      <SplideSlide className={slideClassName} key={index}>
        <ProductCard
          item={item}
          mobileHeartSize={mobileHeartSize}
